...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="dark">
      <body
        className={`${inter.className} ${spaceMono.variable} bg-[#111111] text-[#a1a1aa] antialiased min-h-screen flex flex-col items-center pt-8 pb-12 px-6`}
      >
        <div className="w-full max-w-3xl flex flex-col min-h-screen">
          <nav className="flex justify-between items-center text-sm font-medium text-zinc-500 mb-16 shrink-0">
            <div className="flex gap-6">
              <TransitionLink
                href="/"
                className={`transition-colors hover:text-white ${pathname === "/" ? "text-zinc-100" : ""}`}
              >
                Home
              </TransitionLink>
              <TransitionLink
              ...
              <TransitionLink
                href="/about"
                className={`transition-colors hover:text-white ${pathname === "/about" ? "text-zinc-100" : ""}`}
              >
                About
              </TransitionLink>
            </div>
            <TransitionLink
              href="/cv"
              className={`transition-colors hover:text-white ${pathname === "/cv" ? "text-zinc-100" : ""}`}
            >
              CV
            </TransitionLink>
          </nav>
